...
1. **分句**：將連續單字組合成自然字幕段落
   ...
   - **中文譯文**：每條 `tgt` 的**漢字數不超過 20 個**（英文保留詞不計入）；超過時將原句從語意停頓處拆成兩條，`word_start`/`word_end` 依比例分配給兩條
...
|-------|------|------|------|----------------|
| ZH    | Noto Sans CJK TC | 64 | 白色 | 100px（中文，上方）|
| EN    | Noto Sans | 28 | 淡灰 #CCCCCC | 68px（原文，底部）|
